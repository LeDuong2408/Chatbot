...
class PDFLoader(BaseLoader):
    ...
    def __call__(self, pdf_files: List[str], **kwargs):
        num_processes = min(self.num_processes, kwargs["wokers"])
...
class TextSplitter:
    ...
    def __call__(self, documents):
        return self.splitters.split_documents(documents)
...
class Loader:
    ...
    def load(self, pdf_files: Union[str, List[str]], wokers: int = 1):
        if isinstance(pdf_files, str):
            pdf_files = [pdf_files]
        doc_loaded = self.doc_loader(pdf_files, wokers=wokers)
        doc_split = self.doc_splitter(doc_loaded)
        return doc_split
    
    def load_dir(self, dir_path: str, wokers: int = 1):
        ...
        return self.load(files, wokers=wokers)